...
function _buildPickerUI() {
  ...
  panel.querySelector('#havril-theme-toggle')?.addEventListener('click', () => {
    ...
    try { chrome.storage.sync.set({ havrilTheme: next ? 'dark' : 'light' }); } catch (_e) {}
    _applyTokens(next);
    ...
    if (newPanel) {
      ...
      sendToBackground('LIST_MEMORIES', {}).then((data) => {
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    if (q.length === 0) { loadAllMemories(); }
    else if (q.length >= 2) { searchTimeout = setTimeout(() => searchMemories(q), 350); }
  });
  ...
  async function loadAllMemories() {
    ...
    try {
      ...
      showPlaceholder(err.message?.includes('token not set')
...
function _renderInto(container, memories) {
  ...
  container.innerHTML = memories.map((m, i) => {
    ...
    const preview = (m.content || '').length > 110
      ? m.content.slice(0, 110) + '…' : m.content || '';
...
function showToast(message, type = 'info') {
  ...
  const s = (_darkTheme ? DARK_STYLES : LIGHT_STYLES)[type] || LIGHT_STYLES.info;
